...
network = Network(model, n_filter_width, n_filters)
...
with tf.Session() as sess:
    ...
    weights_tf = network.get_tf_weights()
    ...
    for t in range(model.n_runs):
        ...
        w_vals, v_dum_vals, v_vals, a_vals, weights, cost, _ = sess.run([w_ph, v_dum_ph, v_ph, a_ph, \
                                                        weights_tf, cost_op, optimizer], feed_dict=feed_dict)
        print ("%d) Cost: " % (t), cost)
